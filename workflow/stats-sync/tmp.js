...
var readCount = (runtime2, client, contractAddr, abi, functionName) => {
...
var onCronTrigger = async (runtime2, _payload) => {
  runtime2.log("Cron: stats-sync triggered");
  let totalAirdrops = 0n;
  let totalLaunches = 0n;
  const chains = [
    { name: "World Chain", selector: WORLD_CHAIN, humanDrop: runtime2.config.humanDropWorldChain, fairLaunch: runtime2.config.fairLaunchWorldChain },
    { name: "Arbitrum Sepolia", selector: ARB_SEPOLIA, humanDrop: runtime2.config.humanDropArbSepolia, fairLaunch: runtime2.config.fairLaunchArbSepolia },
    { name: "Base Sepolia", selector: BASE_SEPOLIA, humanDrop: runtime2.config.humanDropBaseSepolia, fairLaunch: runtime2.config.fairLaunchBaseSepolia }
  ];
  for (const chain of chains) {
    try {
      const client = new ClientCapability(chain.selector);
      const airdropCount = readCount(runtime2, client, chain.humanDrop, HumanDropAbi, "nextAirdropId");
      const launchCount = readCount(runtime2, client, chain.fairLaunch, FairLaunchAbi, "nextLaunchId");
      runtime2.log(`${chain.name}: ${airdropCount} airdrops, ${launchCount} launches`);
      totalAirdrops += airdropCount;
      totalLaunches += launchCount;
    } catch (err) {
      runtime2.log(`${chain.name} error: ${err}`);
    }
  }
  runtime2.log(`Total: ${totalAirdrops} airdrops, ${totalLaunches} launches`);
  const worldChainClient = new ClientCapability(WORLD_CHAIN);
  ...
  const report2 = runtime2.report(prepareReportRequest(updateCalldata)).result();
  const writeResult = worldChainClient.writeReport(runtime2, {
    receiver: runtime2.config.mannaIndexWorldChain,
    report: report2,
    gasConfig: { gasLimit: "300000" }
  }).result();
  if (writeResult.txStatus !== "TX_STATUS_SUCCESS") {
    runtime2.log(`STATS_UPDATE_FAILED: ${writeResult.txStatus}`);
    return {
      success: false,
      error: `Update failed: ${writeResult.txStatus}`
    };
  }
  runtime2.log(`Stats updated, txHash: ${writeResult.txHash ? bytesToHex(writeResult.txHash) : "0x"}`);
  return {
    success: true,

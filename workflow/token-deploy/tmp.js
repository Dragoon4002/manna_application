...
var onHttpTrigger = async (runtime2, payload) => {
  ...
  runtime2.log(`Deploy token: ${request.name} (${request.symbol}) on ${request.targetChain}`);
  let chainSelector;
  let factoryAddress;
  switch (request.targetChain) {
    case "world-chain":
      chainSelector = WORLD_CHAIN;
      factoryAddress = runtime2.config.tokenFactoryWorldChain;
      break;
    case "arbitrum-sepolia":
      chainSelector = ARB_SEPOLIA;
      factoryAddress = runtime2.config.tokenFactoryArbSepolia;
      break;
    case "base-sepolia":
      chainSelector = BASE_SEPOLIA;
      factoryAddress = runtime2.config.tokenFactoryBaseSepolia;
      break;
    default:
  ...
  runtime2.log(`Deploying via factory at ${factoryAddress}`);
  const report2 = runtime2.report(prepareReportRequest(deployCalldata)).result();
  ...
  runtime2.log(`Deploy tx status: ${writeResult.txStatus}`);

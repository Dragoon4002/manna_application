...
var getLaunchCount = (runtime2, client, contractAddr) => {
...
var getLaunch = (runtime2, client, contractAddr, launchId) => {
  const calldata = encodeFunctionData({
    abi: FairLaunchAbi,
...
var finalizeLaunch = (runtime2, client, contractAddr, launchId) => {
  const calldata = encodeFunctionData({
    abi: FairLaunchAbi,
  ...
  if (writeResult.txStatus !== "TX_STATUS_SUCCESS") {
    runtime2.log(`FINALIZE_FAILED: launchId=${launchId}, status=${writeResult.txStatus}`);
    return false;
  }
  runtime2.log(`Finalized launchId=${launchId}, txHash=${writeResult.txHash ? bytesToHex(writeResult.txHash) : "0x"}`);
  return true;
};
var processChain = (runtime2, chainName, chainSelector, contractAddr) => {
  ...
  try {
    const launchCount = getLaunchCount(runtime2, client, contractAddr);
    runtime2.log(`${chainName}: ${launchCount} launches total`);
    for (let i2 = 0n;i2 < launchCount; i2++) {
      const launch = getLaunch(runtime2, client, contractAddr, i2);
      if (!launch.finalized && launch.endTime < now) {
        runtime2.log(`${chainName}: launchId=${i2} ended, finalizing`);
        if (finalizeLaunch(runtime2, client, contractAddr, i2)) {
          finalizedCount++;
        }
      }
    }
...
var onCronTrigger = async (runtime2, _payload) => {
  runtime2.log("Cron: fair-launch-finalize triggered");
  let totalFinalized = 0;
  totalFinalized += processChain(runtime2, "World Chain", WORLD_CHAIN, runtime2.config.fairLaunchWorldChain);
  totalFinalized += processChain(runtime2, "Arbitrum Sepolia", ARB_SEPOLIA, runtime2.config.fairLaunchArbSepolia);
  totalFinalized += processChain(runtime2, "Base Sepolia", BASE_SEPOLIA, runtime2.config.fairLaunchBaseSepolia);
  runtime2.log(`Total finalized: ${totalFinalized}`);
  return {
    success: true,

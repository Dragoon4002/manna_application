...
var onHttpTrigger = async (runtime2, payload) => {
  ...
  runtime2.log(`Create airdrop: token=${request.token} on ${request.targetChain}`);
  let chainSelector;
  let humanDropAddress;
  switch (request.targetChain) {
    case "world-chain":
      chainSelector = WORLD_CHAIN;
      humanDropAddress = runtime2.config.humanDropWorldChain;
      break;
    case "arbitrum-sepolia":
      chainSelector = ARB_SEPOLIA;
      humanDropAddress = runtime2.config.humanDropArbSepolia;
      break;
    case "base-sepolia":
      chainSelector = BASE_SEPOLIA;
      humanDropAddress = runtime2.config.humanDropBaseSepolia;
      break;
    default:
  ...
  runtime2.log(`Creating airdrop via HumanDrop at ${humanDropAddress}`);
  const report2 = runtime2.report(prepareReportRequest(createCalldata)).result();
  ...
  runtime2.log(`Airdrop created, tx status: ${writeResult.txStatus}`);

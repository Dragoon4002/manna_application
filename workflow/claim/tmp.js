...
var confidentialHttp = new ClientCapability2;
var onHttpTrigger = async (runtime2, payload) => {
  ...
  runtime2.log(`Claim: airdrop=${request.airdropId} nullifier=${request.proof.nullifier_hash}`);
  const verifyResp = confidentialHttp.sendRequest(runtime2, {
    vaultDonSecrets: [{ key: "WORLD_ID_API_KEY", namespace: "humandrop" }],
    request: {
      url: `https://developer.worldcoin.org/api/v2/verify/${runtime2.config.worldIdAppId}`,
      method: "POST",
      multiHeaders: {
  ...
  const verifyData = json(verifyResp);
  if (!verifyData.success) {
    throw new Error("World ID proof verification failed");
  }
  runtime2.log("World ID proof verified");
  const arbSepolia = new ClientCapability(ARB_SEPOLIA);
  const hasClaimedData = encodeFunctionData({
    abi: hasClaimedAbi,
    functionName: "hasClaimed",
    args: [BigInt(request.airdropId), BigInt(request.proof.nullifier_hash)]
  });
  const readResult = arbSepolia.callContract(runtime2, {
    call: encodeCallMsg({
      from: ZERO_ADDR,
      to: runtime2.config.humanDropAddress,
      data: hasClaimedData
    }),
    blockNumber: LATEST_BLOCK_NUMBER
  }).result();
  const readHex = bytesToHex(readResult.data);
  ...
  runtime2.log("Not yet claimed — proceeding");
  ...
  const claimReport = runtime2.report(prepareReportRequest(claimCalldata)).result();
  const claimWrite = arbSepolia.writeReport(runtime2, {
    receiver: runtime2.config.humanDropAddress,
    report: claimReport,
    gasConfig: { gasLimit: "500000" }
  }).result();
  runtime2.log(`Claim tx status: ${claimWrite.txStatus}`);
  const registerCalldata = encodeFunctionData({
    abi: registerHumanAbi,
    functionName: "registerHuman",
    args: [BigInt(request.proof.nullifier_hash), verificationLevel]
  });
  const registerReport = runtime2.report(prepareReportRequest(registerCalldata)).result();
  arbSepolia.writeReport(runtime2, {
    receiver: runtime2.config.worldIdVerifierAddress,
    report: registerReport,
    gasConfig: { gasLimit: "200000" }
  }).result();
  runtime2.log("Human registered on WorldIDVerifier");
  return {
    success: true,

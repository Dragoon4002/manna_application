...
var getTokenBalance = (runtime2, client, tokenAddr, wallet) => {
...
var onHttpTrigger = async (runtime2, payload) => {
  ...
  runtime2.log(`Portfolio aggregate for wallet: ${request.wallet}`);
  const balances = [];
  for (const token of request.tokens) {
    try {
      const chainSelector = getChainSelector(token.chain);
      const client = new ClientCapability(chainSelector);
      const balance = getTokenBalance(runtime2, client, token.address, request.wallet);
      balances.push({
        token: token.address,
        symbol: token.symbol,
      ...
      runtime2.log(`${token.symbol} on ${token.chain}: ${balance.toString()}`);
    } catch (err) {
      runtime2.log(`Error reading ${token.symbol} on ${token.chain}: ${err}`);
      balances.push({
        token: token.address,
        symbol: token.symbol,
        balance: "0",

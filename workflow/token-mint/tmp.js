...
var onHttpTrigger = async (runtime2, payload) => {
  ...
  runtime2.log(`Mint ${request.amount} tokens to ${request.to} on ${request.targetChain}`);
  let chainSelector;
  ...
  const mintCalldata = encodeFunctionData({
    abi: mintAbi,
    functionName: "mint",
    args: [request.to, BigInt(request.amount)]
  });
  runtime2.log(`Minting via token at ${request.tokenAddress}`);
  const report2 = runtime2.report(prepareReportRequest(mintCalldata)).result();
  ...
  runtime2.log(`Mint tx status: ${writeResult.txStatus}`);

...
var WORLD_CHAIN = ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia-worldchain-1"];
var ARB_SEPOLIA = ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia-arbitrum-1"];
var BASE_SEPOLIA = ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia-base-1"];
var ZERO_ADDR = "0x0000000000000000000000000000000000000000";
var getAirdropCount = (runtime2, client, contractAddr) => {
  const calldata = encodeFunctionData({
    abi: HumanDropAbi,
    functionName: "nextAirdropId"
  });
  const result = client.callContract(runtime2, {
    call: encodeCallMsg({
      from: ZERO_ADDR,
      to: contractAddr,
      data: calldata
...
var getAirdrop = (runtime2, client, contractAddr, airdropId) => {
  const calldata = encodeFunctionData({
    abi: HumanDropAbi,
...
var reclaimAirdrop = (runtime2, client, contractAddr, airdropId) => {
  const calldata = encodeFunctionData({
    abi: HumanDropAbi,
  ...
  const writeResult = client.writeReport(runtime2, {
    receiver: contractAddr,
    report: report2,
    gasConfig: { gasLimit: "300000" }
  }).result();
  if (writeResult.txStatus !== "TX_STATUS_SUCCESS") {
    runtime2.log(`RECLAIM_FAILED: airdropId=${airdropId}, status=${writeResult.txStatus}`);
    return false;
  }
  runtime2.log(`Reclaimed airdropId=${airdropId}, txHash=${writeResult.txHash ? bytesToHex(writeResult.txHash) : "0x"}`);
  return true;
};
var processChain = (runtime2, chainName, chainSelector, contractAddr) => {
  runtime2.log(`Processing ${chainName} at ${contractAddr}`);
  const client = new ClientCapability(chainSelector);
  const now = BigInt(Math.floor(Date.now() / 1000));
  let reclaimedCount = 0;
  try {
    const airdropCount = getAirdropCount(runtime2, client, contractAddr);
    runtime2.log(`${chainName}: ${airdropCount} airdrops total`);
    for (let i2 = 0n;i2 < airdropCount; i2++) {
      const airdrop = getAirdrop(runtime2, client, contractAddr, i2);
      if (airdrop.expiry < now && airdrop.totalClaimed < airdrop.maxClaims) {
        runtime2.log(`${chainName}: airdropId=${i2} expired, reclaiming`);
        if (reclaimAirdrop(runtime2, client, contractAddr, i2)) {
          reclaimedCount++;
        }
      }
    }
  } catch (err) {
    runtime2.log(`${chainName} error: ${err}`);
  }
  return reclaimedCount;
};
var onCronTrigger = async (runtime2, _payload) => {
  runtime2.log("Cron: airdrop-reclaim triggered");
  let totalReclaimed = 0;
  totalReclaimed += processChain(runtime2, "World Chain", WORLD_CHAIN, runtime2.config.humanDropWorldChain);
  totalReclaimed += processChain(runtime2, "Arbitrum Sepolia", ARB_SEPOLIA, runtime2.config.humanDropArbSepolia);
  totalReclaimed += processChain(runtime2, "Base Sepolia", BASE_SEPOLIA, runtime2.config.humanDropBaseSepolia);
  runtime2.log(`Total reclaimed: ${totalReclaimed}`);
  return {
    success: true,

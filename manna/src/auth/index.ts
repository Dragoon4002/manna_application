...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  providers: [
    // Debug mode: accept any ETH address without MiniKit
    Credentials({
      id: 'debug',
      name: 'Debug (local only)',
      credentials: {
        address: { label: 'ETH Address', type: 'text' },
      },
      // @ts-expect-error NextAuth credentials types are loosely typed
      authorize: async ({ address }: { address: string }) => {
        if (process.env.DEBUG_MODE !== 'true') return null;
        if (!address?.startsWith('0x')) return null;
        return { id: address, walletAddress: address, username: 'debug-user', profilePictureUrl: '' };
      },
    }),
    Credentials({
